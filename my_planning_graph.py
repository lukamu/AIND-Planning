...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Set a_level like in create_graph for s_levels
        self.a_levels.append(set())

        for action in self.all_actions:
            # Build PgNode for this action
            node_a = PgNode_a(action)
            # Check if for this action the node is reachable, and preconditions have to be satisfied by the previous  level
            if node_a.prenodes.issubset(self.s_levels[level]):
                # Create the linked list between levels
    ...
    def add_literal_level(self, level):
        ...
        # every A node in the previous level (level - 1) has a list of S nodes in effnodes that represent the effect
        ...
        for node_a in self.a_levels[level - 1]:
            for node_effect in node_a.effnodes:
                # Create the linked list creating and connecting all of the new S nodes as children of all the A nodes
                # that could produce them, and likewise add the A nodes to the parent sets of the S nodes.
                node_a.children.add(node_effect)
                node_effect.parents.add(node_a)
                # These literals will all be part of the new S level
                self.s_levels[level].add(node_effect)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        #Test the first Action Node
        # Check if some positive literals are canceled by negative literals
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Check if some positive literals in effect list are canceled by negative literals in preconditional list
        for action_effect in node_a1.action.effect_add:
        ...
        # Check if some negative literals in effect list are canceled by positive literals in preconditional list
